...
import { addSource, addSubject, getAssessmentRun, getReport, getWorkspace, removeSubject, renameSubject, resetSample, runInvestigation, sendWorkspaceMessage, updateCaseTitle } from "../../../lib/case-service";
...
export default function CasePage() {
  ...
  return <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
    ...
    <section aria-live="polite" className="mx-auto mt-5 max-w-6xl rounded-xl border bg-white p-4"><h2 className="font-black">Assessment status</h2><p className="mt-1 text-sm">{copy(workspace.assessment.state)}</p>{statusError && <div className="mt-3 flex justify-between rounded bg-red-50 p-3 text-sm text-red-800"><span>{statusError}</span>{handle.current && <button onClick={() => void poll(handle.current as string)} className="font-bold underline">Retry status check</button>}</div>}</section>
