...
import { addSource, addSubject, downloadTrace, getAssessmentRun, getReport, getWorkspace, removeSubject, renameSubject, resetSample, runInvestigation, sendWorkspaceMessage, updateCaseTitle } from "../../../lib/case-service";
import type { CaseWorkspaceState, ReportResponse } from "../../../lib/types";
...
const shortestSummaryPrompt = "Give me the shortest possible summary of the current report in exactly one sentence. State only the current result and the single most important remaining uncertainty. Do not include evidence details, background, next steps, headings, or bullets.";
const quickPrompts = ["What remains uncertain?", "What evidence would be useful next?", "Why is the current conclusion limited?", "Is it reasonable to stop here?", "Shortest summary"];
...
export default function CasePage() {
  ...
  const [traceDownloadBusy, setTraceDownloadBusy] = useState(false);
  ...
  const helpTranscript = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const latest = helpTranscript.current?.lastElementChild as HTMLElement | null;
  ...
  async function downloadRunTrace(runHandle: string) { if (traceDownloadBusy) return; setTraceDownloadBusy(true); try { await downloadTrace(caseId, runHandle); } catch (exc) { setError(exc instanceof Error ? exc.message : "Could not download the trace."); } finally { setTraceDownloadBusy(false); } }
  ...
  const traceReady = workspace.assessment.latestAttempt?.traceReady === true;
  return <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
    <header className="mx-auto flex max-w-6xl items-start justify-between border-b pb-5">
      <div className="flex items-start gap-3"><img src="/caselens-logo.png" alt="CaseLens" className="h-10 w-auto object-contain"/><div><Link href="/cases" className="font-black uppercase tracking-widest text-blue-700">SimplifyNext</Link>{editingTitle && !sample ? <div className="mt-2 flex gap-2"><label htmlFor="case-title" className="sr-only">Case name</label><input id="case-title" value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} maxLength={160} autoFocus className="rounded border px-2 py-1 text-2xl font-black"/><button disabled={titleBusy} onClick={() => void saveTitle()} className="rounded bg-blue-700 px-3 py-1 text-sm font-bold text-white">{titleBusy ? "Saving…" : "Save"}</button><button disabled={titleBusy} onClick={() => { setDraftTitle(workspace.title); setEditingTitle(false); }} className="rounded border px-3 py-1 text-sm font-bold">Cancel</button></div> : <div className="mt-2 flex items-center gap-3"><h1 className="text-2xl font-black">{workspace.title}</h1>{!sample && <button onClick={() => setEditingTitle(true)} className="text-sm font-bold text-blue-700 underline">Edit</button>}{sample && <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold uppercase text-blue-700">Sample</span>}</div>}</div></div>
