...
import { applyAwsCredentials, clearAwsCredentials, getAwsCredentialStatus, getWorkspace, markUnavailable, provideEvidence, runInvestigation } from "../lib/case-service";
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-3xl px-5 py-7">
      <div className="mb-4 rounded-lg border border-slate-200 bg-white px-5 py-4 text-sm text-slate-700"><div className="flex flex-wrap gap-x-6 gap-y-2"><span><strong>Case status:</strong> {workspace.caseStatus}</span><span><strong>Runtime:</strong> {workspace.runtimeStatus}</span><span><strong>Actor:</strong> {workspace.currentActor}</span><span><strong>Revision:</strong> {workspace.caseRevision ?? "—"}</span><span><strong>Last step:</strong> {workspace.lastTraceStep ?? "—"}</span></div>{workspace.lastUpdatedAt && <div className="mt-2 text-xs text-slate-500">Last backend update: {workspace.lastUpdatedAt}</div>}</div>
      ...
      <div className={`mb-4 rounded-lg border px-5 py-4 ${workspace.runtimeStatus === "FAILED" ? "border-red-200 bg-red-50" : "border-slate-200 bg-white"}`}><div className="font-bold text-slate-900">{runtime?.[0]}</div><p className="mt-1 text-sm text-slate-600">{runtime?.[1]}</p>{workspace.runtimeStatus === "IDLE" && <button disabled={starting} onClick={startInvestigation} className="mt-3 rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{starting ? "Starting…" : "Run investigation"}</button>}{workspace.runtimeStatus === "FAILED" && <button disabled={starting} onClick={startInvestigation} className="mt-3 rounded-lg border border-blue-700 px-4 py-2 text-sm font-bold text-blue-800 disabled:opacity-50">{starting ? "Starting…" : "Retry investigation"}</button>}{workspace.lastError && <div className="mt-3 text-sm text-red-800"><strong>CRITICAL ERROR</strong><div>{workspace.lastError.failure_category}</div><div>{workspace.lastError.message}</div>{workspace.lastError.step != null && <div>Step: {workspace.lastError.step}</div>}{rawTraceUrl && <div className="mt-2 flex gap-4"><a href={rawTraceUrl} target="_blank" rel="noreferrer" className="font-bold underline">Open raw trace</a><a href={`/debug/cases/${encodeURIComponent(workspace.caseId)}/runs/${encodeURIComponent(latestRun!.run_id)}`} target="_blank" rel="noreferrer" className="font-bold underline">View run artifacts</a></div>}</div>}</div>
      ...
      <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4" aria-label="Workspace composer"><p className="mb-3 text-xs text-slate-500">Manual input is disabled in this testing surface.</p><div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-2 opacity-70"><button disabled aria-label="Attachments disabled" className="rounded-md px-2 py-2 text-slate-400">⌕</button><input disabled placeholder="Add case note or material…" className="min-w-0 flex-1 bg-transparent px-1 text-sm text-slate-500 outline-none"/><button disabled aria-label="Send disabled" className="rounded-md bg-slate-200 px-3 py-2 text-slate-400">➤</button></div></section>
    </div>
