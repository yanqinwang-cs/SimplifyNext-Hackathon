...
function runtimeCopy(status: CaseWorkspaceState["runtimeStatus"]) {
  const copy = {
    RUNNING: ["Investigation is running…", "The finite Investigator assessment is in progress."],
    COMPLETED: ["Investigation completed.", "The finite assessment is complete."],
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-3xl px-5 py-7">
      ...
      <div className={`mb-4 rounded-lg border px-5 py-4 ${workspace.runtimeStatus === "FAILED" ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white"}`}><div className="font-bold text-slate-900">{runtime?.[0]}</div><p className="mt-1 text-sm text-slate-600">{runtime?.[1]}</p>{(workspace.runtimeStatus === "IDLE" || workspace.runtimeStatus === "COMPLETED" || workspace.runtimeStatus === "FAILED") && <button disabled={starting} onClick={startInvestigation} className="mt-3 rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{starting ? "Starting…" : workspace.runtimeStatus === "COMPLETED" ? "Run again" : "Run investigation"}</button>}{workspace.runtimeStatus === "FAILED" && <div className="mt-3 rounded-lg border border-amber-200 bg-white p-3 text-sm text-amber-900"><strong>Investigation recovered or paused safely.</strong><p className="mt-1">No invalid case change was committed. Open Debug to inspect the technical details.</p>{rawTraceUrl && <a href={rawTraceUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block font-bold underline">Open debug trace</a>}</div>}</div>
      {latestRun && <section className="mb-4 rounded-lg border border-slate-200 bg-white px-5 py-4 text-sm"><div className="font-bold text-slate-900">Latest run</div><div className="mt-1 text-slate-700">{latestRun.run_id} · {latestRun.final_runtime_status ?? workspace.runtimeStatus}</div>{latestRun.vnext_furthest_conclusion && <p className="mt-2 text-slate-600">{latestRun.vnext_furthest_conclusion}</p>}{rawTraceUrl && <div className="mt-2 flex gap-4"><a href={rawTraceUrl} target="_blank" rel="noreferrer" className="font-semibold text-blue-700 underline">Open raw trace</a><a href={`/debug/cases/${encodeURIComponent(workspace.caseId)}/runs/${encodeURIComponent(latestRun.run_id)}`} target="_blank" rel="noreferrer" className="font-semibold text-blue-700 underline">View run artifacts</a></div>}</section>}
      {(workspace.workspaceEvents?.length ?? 0) > 0 && <section className="mb-4 rounded-lg border border-slate-200 bg-white px-5 py-4 text-sm" aria-label="Workspace events"><div className="font-bold text-slate-900">Recent workspace events</div><div className="mt-3 space-y-2">{workspace.workspaceEvents?.map((event) => <div key={event.event_id} className="rounded border border-slate-100 bg-slate-50 p-2 text-slate-700">{event.human_summary}{event.run_id && <span className="ml-1 text-xs text-slate-500">({event.run_id})</span>}</div>)}</div></section>}
