...
export default function ReportPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-10 text-slate-900"><article className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
    ...
    {report.reportState === "available" && assessmentView && <>
      ...
      <section className="mt-8"><h2 className="text-xl font-black">Findings by student</h2><div className="mt-4 space-y-8">{assessmentView.students.map((student) => <section id={student.sectionHandle} key={student.sectionHandle} className="scroll-mt-6 border-t border-slate-200 pt-5"><h3 className="text-lg font-bold">{student.displayName}</h3>{student.violations.map((item) => <div key={item.label} className="mt-4 rounded-lg bg-slate-50 p-4"><div className="flex flex-wrap justify-between gap-3"><strong>{item.label}</strong><span className="text-xs font-bold uppercase">{statusLabels[item.status] ?? item.status.replaceAll("_", " ")}</span></div><p className="mt-2 text-sm leading-6">{item.reasoningSummary}</p><div className="mt-4 grid gap-4 text-sm sm:grid-cols-3"><div><strong>Supporting material</strong><ul className="mt-2 list-disc pl-5">{item.supportingMaterial.length ? item.supportingMaterial.map((material, index) => <Material key={`${material.statement}-${index}`} material={material} caseId={caseId} runHandle={assessmentView.runHandle} />) : <li>No supporting material recorded.</li>}</ul></div><div><strong>Conflicting material</strong><ul className="mt-2 list-disc pl-5">{item.conflictingMaterial?.length ? item.conflictingMaterial.map((material, index) => <Material key={`${material.statement}-${index}`} material={material} caseId={caseId} runHandle={assessmentView.runHandle} />) : <li>No conflicting material recorded.</li>}</ul></div><div><strong>Limiting material</strong><ul className="mt-2 list-disc pl-5">{item.limitingMaterial.length ? item.limitingMaterial.map((material, index) => <Material key={`${material.statement}-${index}`} material={material} caseId={caseId} runHandle={assessmentView.runHandle} />) : <li>No limiting material recorded.</li>}</ul></div></div>{item.unresolvedPoints.length > 0 && <div className="mt-4"><strong>Unresolved points</strong><ul className="mt-1 list-disc pl-5">{item.unresolvedPoints.map((point) => <li key={point}>{point}</li>)}</ul></div>}</div>)}{(student.alternativeExplanations ?? []).length > 0 && <div className="mt-4 rounded-lg border border-slate-200 p-4 text-sm"><strong>Alternative explanations</strong><ul className="mt-2 list-disc pl-5">{student.alternativeExplanations?.map((item) => <li key={item.statement}>{item.statement}</li>)}</ul></div>}<p className="mt-4 text-sm font-semibold">Furthest justified conclusion: {student.furthestConclusion}</p>{student.suggestedNextStep && <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-4 text-sm"><strong>Suggested next step</strong><p className="mt-2 leading-6">{student.suggestedNextStep}</p></div>}</section>)}</div></section>
    </>}
