...
def build_prompt(run_input: VNextRunInput) -> str:
    ...
    return "\n".join(
        [
            ...
            "Return meaning-level semantic items only. Do not emit implementation details, internal identifiers, internal links, validator terms, or registry details. Every semantic item must declare about_subject_ids. Evidence statements use basis_source_ids; propositions use basis_item_refs for earlier semantic items. Supporting, conflicting, limiting, and alternative references must name semantic item local_refs.",
