...
def build_prompt(run_input: VNextRunInput) -> str:
    ...
    return "\n".join(
        [
            ...
            "A one-student source cannot support another student. Do not stitch separate A-only and B-only sources into a relationship; cannot combine incompatible private student scopes.",
            ...
            "Evidence statements inherit legal scope from their cited source; who is mentioned in a source does not change that scope. Every semantic item must explicitly list the configured student or students it concerns when its kind requires about_subject_ids; propositions and hypotheses use that field. A multi-student item must use one admitted joint source for the existing participant context; never widen private material to another student. If separate private sources concern A and B, create separate semantic items for A and B. Do not summarize separate A and B sources into one evidence_statement.",
            ...
            "\nSEMANTIC ITEM CONTRACT\nsemantic_items is the only definition table. Define each local_ref once and reuse it in references. evidence_statement: records what same-scope sources say; use basis_source_ids only; do not use semantic item refs; legal scope comes from cited sources. proposition: an inference from semantic items; use basis_item_refs only; never cite raw sources; case context may narrow to one compatible student or relationship, but never combine different private student scopes. hypothesis: a possible explanation, not an established fact; use about_subject_ids; it does not require evidential support; multiple students require one existing exact relationship context. Do not create an uncertainty semantic item; put remaining uncertainty in unresolved_points.",
            "MATERIAL ROLES\nsupporting = tends toward the violation; conflicting = affirmatively conflicts with it; limiting = restricts the inference without affirmatively contradicting it; alternative = a competing hypothesis.",
            "CROSS-STUDENT RULE\nDo not combine evidence from different student-specific source scopes into one semantic item. If Candidate A and Candidate B have separate evidence, create separate semantic items. Cross-student similarity found only by comparing separately scoped sources remains separate student-scoped observations. Do not create a joint proposition from those sources. Do not treat adjacency or a case-wide seating plan as relationship evidence.",
            "TINY EXAMPLE\nBAD: one evidence_statement uses A_invigilator_report and B_invigilator_report. GOOD: separate evidence_statement items use A_invigilator_report and B_invigilator_report. BAD: an A-private item plus a B-private item becomes one joint proposition. GOOD: retain separate propositions unless genuine relationship-scoped basis exists.",
