...
def build_prompt(run_input: VNextRunInput) -> str:
    ...
    return "\n".join(
        [
            ...
            "Return meaning-level semantic items only. Do not emit implementation details, internal identifiers, internal links, validator terms, or registry details. Every semantic item must declare about_subject_ids. Define every semantic item exactly once in semantic_items; semantic_items is the only definition table for semantic objects. Reuse each local_ref wherever that item is referenced and do not redefine an existing local_ref. Evidence statements use basis_source_ids; propositions use basis_item_refs. Alternative explanations are hypothesis items in semantic_items and are referenced using alternative_item_refs. Supporting, conflicting, limiting, and alternative references must name semantic item local_refs.",
