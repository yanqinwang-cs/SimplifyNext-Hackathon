...
def _scope_for_item(item: SemanticItem, run_input: VNextRunInput) -> GraphScope:
...
def compile_semantic_assessment(assessment: InvestigatorSemanticAssessment, run_input: VNextRunInput, *, preset: AssessmentRulePreset | None = None) -> InvestigatorAssessment:
    ...
    scopes = {item.local_ref: _scope_for_item(item, run_input) for item in items}
    ...
    for subject_id in expected_subjects:
        ...
        for violation in violations:
            refs = violation.supporting_item_refs + violation.limiting_item_refs + violation.alternative_item_refs
            ...
            for ref in violation.supporting_item_refs:
                if by_ref[ref].kind not in {SemanticItemKind.EVIDENCE_STATEMENT, SemanticItemKind.PROPOSITION}:
                    raise SemanticValidationError(f"supporting material {ref!r} must be evidence or proposition")
                if by_ref[ref].about_subject_ids and subject_id not in by_ref[ref].about_subject_ids:
    ...
    for subject_id in expected_subjects:
        ...
        for violation in source_subject.violation_assessments:
            ...
            for item_ref in violation.limiting_item_refs:
                updates.append(AddConflictCommand(source_node_id=local_refs[item_ref], target_node_id=target_ref, reason="Attach explicitly declared limiting material to the evaluation target."))
            compiled_violations.append(ViolationAssessment(violation_id=violation.violation_id, status=violation.status, supporting_node_ids=[local_refs[item_ref] for item_ref in violation.supporting_item_refs], mitigating_node_ids=[local_refs[item_ref] for item_ref in violation.limiting_item_refs], unresolved_points=violation.unresolved_points, reasoning_summary=violation.reasoning_summary, confidence=violation.confidence))
