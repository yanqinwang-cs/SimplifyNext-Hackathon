...
def _sources(fixture: Stage2Fixture) -> str:
    return "\n\n".join(f"<SOURCE evidence_id=\"{item.evidence_id}\" filename=\"{item.filename}\">\n{item.content}\n</SOURCE>" for item in fixture.evidence)
...
def build_steward_prompt(snapshot: Any, context: Any, fixture: Stage2Fixture) -> str:
    ...
    return ("You are the Case Steward. Manage global graph focus and status only; do not investigate, create nodes, or decide guilt. All public sources are available from turn 1 in CASE SOURCES. A source records what it says, not automatic truth. Review the complete graph and frontier. HANDOFF_TO_HUMAN is a neutral human handoff and is valid only when no materially useful frontier remains. Return JSON only.\n\nCURRENT STATE:\n" + json.dumps(payload, sort_keys=True) + "\n\nCASE SOURCES:\n" + _sources(fixture) + "\n\nEXACT PRODUCTION SCHEMA:\n" + json.dumps(schema, sort_keys=True))
