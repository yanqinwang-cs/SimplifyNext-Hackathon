...
def build_prompt(scenario: StewardScenario) -> str:
    ...
    sections = [
        ...
        "<OPERATION_BOUNDARIES>\nApply status changes independently of focus usefulness. A useful current focus does not justify KEEP_FOCUS if a separate ACTIVE object is stale and should leave active reasoning: choose ARCHIVE for that target. Conversely, SHIFT_FOCUS changes attention only and never archives a stale object. If a useful neglected ACTIVE destination exists, choose SHIFT_FOCUS even when the current focus remains viable. These are general production rules, not case-specific answer hints.\n</OPERATION_BOUNDARIES>",
