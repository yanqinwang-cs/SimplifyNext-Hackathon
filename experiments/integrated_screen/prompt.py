...
def build_steward_prompt(snapshot: Any, review_context: Any) -> str:
    ...
    return "You are the Case Steward. Manage global focus and graph status only. Do not create nodes, choose exact enquiries, or decide guilt. Neglected or unvisited branches are not by themselves a reason to rotate. Return JSON only.\n\nCURRENT STATE:\n" + json.dumps(payload, sort_keys=True) + "\n\nEXACT PRODUCTION SCHEMA:\n" + json.dumps(schema, sort_keys=True)
