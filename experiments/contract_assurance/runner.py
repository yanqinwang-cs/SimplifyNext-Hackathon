...
def run_deterministic(root: Path, output_dir: Path, commit: str = "unknown") -> dict[str, Any]:
    ...
    report = {"inventory": inventory(root, commit), "deterministic": summary, "deterministic_by_contract": summarize_by_contract(results), "coverage_ledger": coverage_ledger(results), "blind_results_included": False, "blind_compliance": blind_compliance_summary(root), "human_review_required": summary.get("s5_candidates", 0) > 0, "semantic_limitations": ["S6 reasoning and semantic quality are not assessed by deterministic schema assurance.", "SmokeResponse is schema-sampled offline; its live Bedrock path is excluded from this offline cycle."]}
