...
def test_deterministic_runner_is_offline_and_writes_inventory(tmp_path: Path):
    ...
    producer = report["blind_compliance"]["producer_compliance"]
    assert producer["evaluations"] == producer["accepted"] + producer["rejected"]
    assert producer["compliance_rate"] == producer["accepted"] / producer["evaluations"]
    assert "Producer compliance:" in render_markdown(report)
    rolling = report["blind_compliance"]["producer_compliance_rolling_window"]
    assert rolling["evaluations"] <= rolling["window_size"]
    assert rolling["evaluations"] == rolling["accepted"] + rolling["rejected"]
...
def test_revision_combined_unknown_parent_and_evidence_is_generated():
    root = Path(__file__).resolve().parents[1]
    sample = json.loads((root / "experiments/contract_assurance/fixtures/RevisionResponse.json").read_text(encoding="utf-8"))["canonical"]
    mutation = next(item for item in mutations(sample, contract="RevisionResponse") if item.name == "unknown_new_hypothesis_parent_and_evidence")
    assert mutation.intended_code == "S3"
    assert json.loads(mutation.raw_output)["new_hypotheses"][0]["parent_id"] == "H999"
    assert json.loads(mutation.raw_output)["new_hypotheses"][0]["conflicted_by"] == ["E999"]
